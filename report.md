...
#Conclusion
Since the Particle Filter approach provided more accurate results in our testing, we decided to make it the default. We left the Kalman Filter code in the submission though in a separate folder. In all, this project gave us excellent insight in the challenges and opportunities involved in creating a robot. Every method has its strengths and weaknesses. As we found it in this case, while the Kalman Filter may hypothetically make the most sense, yet the best results came from the Particle Filter. One must constantly twiddle the inputs and techniques until the best and smartest robot is produced. 
